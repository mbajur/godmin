...
def install_standalone
  ...
  after_bundle do
    ...
    generate("goodmin:install")
    generate("goodmin:resource", "article")
    generate("goodmin:resource", "author")
...
def install_engine
  ...
  inject_into_file "admin/admin.gemspec", before: /^end/ do
    <<-END.strip_heredoc.indent(2)
      s.add_dependency "goodmin", "~> 1.5.0"
    END
  end
  ...
  after_bundle do
    ...
    run_ruby_script("admin/bin/rails g goodmin:install")
    run_ruby_script("admin/bin/rails g goodmin:resource article")
    run_ruby_script("admin/bin/rails g goodmin:resource author")
...
def modify_article_controller(namespace = nil)
  ...
  inject_into_file articles_controller, after: "Goodmin::Resources::ResourceController\n" do
...
def modify_readme
  ...
  readme_text = <<~README
    ...
    This is the source code for a demo application of the [Goodmin](https://github.com/varvet/goodmin) admin framework for Rails.
    It is generated by running `rake sandbox:deploy` inside the Goodmin repo.
  README
...
if ARGV.count > (ARGV - [with_engine, without_engine]).count
  ...
  if yes?("Place goodmin in admin engine?")
    install_engine
  else
    install_standalone
  end
